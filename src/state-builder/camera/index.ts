// Camera parameter types, presets, and snapshot conversion.
//
// CameraData + adjustedCameraPosition were inlined from @mol-view-stories/lib
// to avoid a cross-repo dependency — see git history on _lib.ts for context.

import { Vec3 as MolstarVec3 } from 'molstar/lib/mol-math/linear-algebra.js';
import type { UINode } from '../core/index.ts';
import type { Vec3 } from '../transform/index.ts';
...
export type CameraData = {
  ...
  target: [number, number, number] | MolstarVec3;
  position: [number, number, number] | MolstarVec3;
  up: [number, number, number] | MolstarVec3;
  fov: number;
};
...
export function adjustedCameraPosition(camera: CameraData): [number, number, number] {
  ...
  const delta = MolstarVec3.sub(MolstarVec3(), camera.position as MolstarVec3, camera.target as MolstarVec3);
  return MolstarVec3.scaleAndAdd(MolstarVec3(), camera.target as MolstarVec3, delta, 1 / f) as unknown as [number, number, number];
}
